...
| 김주원 | #7, #8 | Gemini·Mock 요약, AI 인사이트 분석 |
...
7. PR의 GitHub Actions가 모두 성공하고 리뷰 승인을 받은 뒤 병합합니다.
8. `.env`, Gemini API 키, 개인 DB, 로그, 생성 리포트는 GitHub에 올리지 않습니다.
...
실제 Gemini API가 필요하지 않은 개발과 단위 테스트에서는 `mock` Provider를 사용합니다.
...
- 서비스 계층에서 Gemini SDK 응답 객체를 DB에 직접 넘기지 않습니다.
- Provider는 `gemini`와 `mock`이 같은 입력·출력 계약을 따르게 합니다.
- 단위 테스트에서 실제 아이뉴스24 사이트나 Gemini API를 호출하지 않습니다.
...
- 제목 예시: `[AI] Gemini·Mock 뉴스 요약 구현 (#7)`
- 관련 Issue: `Closes #7`
...
8. 테스트 코드가 실제 네트워크나 Gemini API를 호출하지 않는지 확인합니다.
...
- 실제 Gemini API 호출은 최소 1회 수동으로 검증합니다.
...
- 실제 Gemini 호출은 키를 가진 팀원이 제출 전 별도로 확인합니다.
...
- [ ] 실제 사이트와 Gemini API를 단위 테스트에서 호출하지 않았다.
