...
class AnalyzerService:
    ...
    def analyze(self, *, date_from: str | None, date_to: str | None, category: str | None, limit: int | None) -> AnalysisResult | None:
        ...
        prompt = _analysis_prompt(rows, category_counts)
        data = self.provider.generate_json("analysis", prompt, InsightResult, {"category": category or "all", "article_count": len(rows)})
    ...
    def _select_articles(self, date_from, date_to, category):
        ...
        if date_to: clauses.append("published_at <= ?"); params.append(date_to)
...
def _analysis_prompt(rows, category_counts) -> str:
    articles = "\n".join(f"[{row['category']}] {row['title']}\n요약: {row['summary'][:1000]}" for row in rows)
...
def _row_to_result(row) -> AnalysisResult:
    ...
    return AnalysisResult(row["date_from"], row["date_to"], row["category"], row["article_count"], insights, json.loads(row["article_ids"]), json.loads(row["category_counts"]), row["ai_provider"], row["ai_model"], row["id"])
